...
SELECT FIRST_NAME || LAST_NAME AS NAME , DEPARTMENT_ID , JOB_ID FROM EMPLOYEES WHERE DEPARTMENT_ID IN(50);
...
SELECT FIRST_NAME || LAST_NAME AS NAME ,HIRE_DATE , SALARY FROM EMPLOYEES WHERE HIRE_DATE BETWEEN '03/01/01' AND '03/12/12';
...
SELECT FIRST_NAME || LAST_NAME AS NAME,JOB_ID FROM EMPLOYEES WHERE JOB_ID IN ('IT_PROG') OR JOB_ID IN('SA_MAN');
...
SELECT FIRST_NAME || LAST_NAME AS NAME ,JOB_ID FROM EMPLOYEES WHERE MANAGER_ID IS NOT NULL;
--15. 매니저(MAN) 직무에 해당하는 사원의 이름과 직무아이디를 직무아이디 순서대로 출력하세요.
SELECT FIRST_NAME || LAST_NAME AS NAME , JOB_ID FROM EMPLOYEES WHERE MANAGER_ID IS NOT NULL ORDER BY JOB_ID; 
--? 제한시간 : 30분
